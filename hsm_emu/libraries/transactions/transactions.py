...
"""
# change pycoin by btcpy for me 
from btcpy.structs.crypto import PrivateKey

"""
kmilodenisglez

Change my own RegtestDaemonService no utiliza bitcoin library y no request
"""
from .services.myownregtestservice import RegtestDaemonService
...
class Transactions(object):
    ...
    def simple_transaction(self, from_address, to, op_return=None, min_confirmations=6):
        ...
        outputs = [{'address': to_address, 'value': _amount} for to_address, _amount in to]
    ...
    def sign_transaction(self, tx, master_password, path=''):
        ...
        try:
            ...
            PrivateKey.from_wif(master_password, check_network=False)
            return bitin.signall(tx, master_password)
        except Exception as e:
            raise e            
        #except (AttributeError, EncodingError):
            # if its not get the wif from the master secret
        #    return bitcoinfork.signall(tx, BIP32Node.from_master_secret(master_password, netcode=netcode).subkey_for_path(path).wif())
    ...
    def decode(self, tx):
        ...
        Returns:
            decoded transaction
        """
        return self._service.decode_raw_transaction(tx)
