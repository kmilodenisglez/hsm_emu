...
import json
from libraries.utils_wallets import setup, derive, getXPubKey, signMessage, verifyMessage, bip32KeyInfoFromKey
...
class derivation:        
    def POST(self):
        values = web.input(bip32Key={})
        try:
            derivedKey = derive(values['bip32Key'], "m/0'/0'/276'")
            derived_encode = derivedKey.encode(mainnet=False)
            derived_pub_encode = derivedKey.pub().encode(mainnet=False)            
        except Exception as e:
            return json.dumps(
                {
                    'error': True,
        ...
        return json.dumps(
            {
                ...
                'wifDerivedPubKey':derivedKey.key.to_wif(), 
                'hexDerivedPubKey':derivedKey.pub().key.hexlify(), 
